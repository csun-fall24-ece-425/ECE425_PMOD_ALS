...
void PMOD_ALS_Init(void)
{
	...
	// Configure the SSI0 module to capture data on the second clock edge transition
	// by setting the SPH bit (Bit 7) in the CR0 register
	SSI0->CR0 |= 0x0080;
	
	// Configure the SCLK of the SSI0 module to be idle high when inactive
	// by setting the SPO bit (Bit 6) in the CR0 register
	SSI0->CR0 |= 0x0040;
	...
	// Configure the SSI0 module to have a data length of 16 bits
	// by writing a value of 0xF to the DSS field (Bits 3 to 0) in the CR0 register
	SSI0->CR0 |= 0x000F;
...
uint16_t PMOD_ALS_Input_Raw_Data(void)
{
...
{
	// Write a dummy word (0x0000) to enable the CS line and generate SCLK clock cycles
	PMOD_ALS_Output_Data(0x0000);
	
	uint16_t raw_data = PMOD_ALS_Input_Raw_Data();
	
	uint8_t light_level = (raw_data >> 5) & 0xFF;
